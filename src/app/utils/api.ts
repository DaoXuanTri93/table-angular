...
export const API = {
  ...
  checkRefreshToken: async ({ response, url, config, linkParam }) => {
    if (
      response.status === 401 &&
      ...
      if (token) {
        ...
        return await fetch(LINK_API + url + (linkParam && '?' + linkParam), config);
      }
  ...
  responsible: async <T>({
    ...
    response = await API.checkRefreshToken({ response, url, config, linkParam });
