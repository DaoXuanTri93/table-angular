    ...
    switch (rule.type) {
      ...
      case EFormRuleType.Email:
        rules.push(({ value }) => {
          ...
          return t.instant(rule.message ?? 'PleaseEnterAValidEmailAddress');
        });
        break;
      case EFormRuleType.Phone:
        rules.push(({ value }) => {
          if (!value) return '';
            ...
            if (value?.trim().length < 10) return t.instant('PleaseEnterAtLeastCharacters', { min: 10 });
            else if (value?.trim().length > 12) return t.instant('PleaseEnterMaximumCharacters', { max: 12 });
            else return '';
          } else return t.instant('PleaseEnterOnlyNumber');
        });
        break;
      ...
      case EFormRuleType.OnlyText:
        rules.push(({ value }) => {
          ...
          return t.instant(rule.message ?? 'PleaseEnterOnlyText');
        });
        break;
      ...
      case EFormRuleType.Textarea:
        rules.push(({ value }) => {
          if (value && value?.trim()?.length > 500) {
            return t.instant(rule.message ?? 'PleaseEnterMaximumCharacters', { max: 500 });
          }
          return '';
        });
        break;
  ...
  if (item.formItem?.type === EFormType.Number)
    rules.push(({ value }) => {
      if (!value || (/^0$|^-?[1-9]\d*(\.\d+)?$/.test(value) && parseFloat(value) < rule.value)) {
        return t.instant(rule.message ?? 'PleaseEnterMinimumNumber', { min: rule.value });
      }
      return '';
    });
  else {
    ...
    if (!message) {
      if (item.formItem?.type) {
        message = t.instant('PleaseEnterAtLeastNumberCharacters', { min: rule.value });
      } else {
        message = t.instant('PleaseEnterMinimumNumber', { min: rule.value });
      }
    }
    rules.push(({ value }) => {
...
const generateValidMax = ({
  ...
  if (item.formItem?.type === EFormType.Number)
    rules.push(({ value }) => {
      if (!value || (/^0$|^-?[1-9]\d*(\.\d+)?$/.test(value) && parseFloat(value) > rule.value)) {
        return t.instant(rule.message ?? 'PleaseEnterMaximumNumber', { max: rule.value });
      }
      return '';
    });
  else {
    ...
    if (!message) {
      if (item.formItem?.type === EFormType.OnlyNumber) {
        message = t.instant('PleaseEnterMaximumNumberCharacters', { max: rule.value });
      } else {
        message = t.instant('PleaseEnterMaximumNumber', { max: rule.value });
      }
    }
    rules.push(({ value }) => {
      if (!value || value.length > rule.value) return message;
