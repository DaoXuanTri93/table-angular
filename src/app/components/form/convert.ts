...
export const convertFormValue = (columns: IForm[], values: { [selector: string]: any }, exportData = true) => {
  ...
  const date = ({ item, values, exportData }) => {
    if (values[item.name]) {
      if (exportData) {
        values[item.name] = values[item.name].format
          ? values[item.name]
          : format(new Date(values[item.name]), 'YYYY-MM-DD HH:mm:ss');
      } else values[item.name] = new Date(values[item.name]);
    }
  };
  const dateRange = ({ item, values, exportData }) => {
    if (!!values[item.name] || typeof item.name === 'object') {
      if (exportData) {
        values[item.name] = [
          values[item.name][0].format
            ? values[item.name][0]
            : format(startOfDay(new Date(values[item.name][0])), 'YYYY-MM-DD HH:mm:ss'),
          values[item.name][1].format
            ? values[item.name][1]
            : format(endOfDay(values[item.name][1]), 'YYYY-MM-DD HH:mm:ss'),
        ];
      } else values[item.name] = [new Date(values[item.name][0]), new Date(values[item.name][1])];
    }
  };
  ...
  columns
    ...
    .forEach(item => {
      if (item?.formItem?.convert) {
        newValue[item.name] = item.formItem.convert({ value: values[item.name], values });
      } else {
